...
public class CollisionTriggerListener : MonoBehaviour
{
    ...
    {
        var collision = go.GetComponent<CollisionTriggerListener>();
        if (collision == null) go.AddComponent<CollisionTriggerListener>();
        return collision;
    }
    ...
    private Dictionary<EventName, UnityEventBase> m_Events = new Dictionary<EventName, UnityEventBase>();
    ...
    {
        foreach (var item in m_Events.Values)
        {
    ...
    private void Register<T>(EventName name ,UnityAction<T> action)
    {
        UnityEventBase e;
        if(!m_Events.TryGetValue(name,out e))
        {
    ...
    private void UnRegister<T>(EventName name ,UnityAction<T> action)
    {
        UnityEventBase e;
        if (m_Events.TryGetValue(name, out e))
            (e as CollisionEvent<T>).RemoveListener(action);
    }
    private void Dispatch<T>(EventName name,T value)
    {
        UnityEventBase e;
        if (m_Events.TryGetValue(name, out e))
