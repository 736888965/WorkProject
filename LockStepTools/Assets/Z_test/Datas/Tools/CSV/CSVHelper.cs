...
namespace MokaData
{
    public class CSVHelper
    {
        ...
        {
            if (Path.GetExtension(path) != ".csv" || !File.Exists(path))
            {
                Debug.LogError(Path.GetExtension(path) +"传入的文件后缀错误" + path);
                return;
            }
        ...
        public static List<List<string>> Read(string filePath)
        {
            ...
            string content = File.ReadAllText(filePath, UTF8Encoding.UTF8);//读取csv所有的文本内容
